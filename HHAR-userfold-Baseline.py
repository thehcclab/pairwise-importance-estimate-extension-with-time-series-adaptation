...
def main():
    ...
    for i in range(len(USERS)):
        ...
        model= Model.HHARBaseline_Model(device=DEVICE, input_dim=input_dim, classes=classes)
        model.to(DEVICE)
        ...
        model.training_procedure(iteration=EPOCH, train_dataloader=train_dataloader, val_dataloader=val_dataloader, print_cycle=5,path=f"{save_path}/dictionary/intermediate_dicts", loss_func=loss, optimiser=optimiser, train_func=train_func)

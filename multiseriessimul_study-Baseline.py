...
def main():
    ...
    model= MultivariateSimulationStudyBaseline_Model(device=device, input_dim=data.X.shape[:], classes=classes).to(device)
...
if __name__=="__main__":
    ...
    parser.add_argument('-opt', help="option for dataset to use or create. Options 0, 1, 2. Default: \"0\"", default=0)
    args= parser.parse_args()
    args.dir= str(args.dir)
    args.opt= int(args.opt)

    feat= f"multiseriessimul_study{args.opt}-Baseline"

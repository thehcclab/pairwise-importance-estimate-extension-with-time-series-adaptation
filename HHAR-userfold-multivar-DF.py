...
def main():
    ...
    for i in range(len(USERS)):
        ...
        model= Models.Multivariate_DF(device=DEVICE, input_dim=[win_len,input_dim], model=baseline_model, l1_lambda=config["l1_lambda"], l2_lambda=config["l2_lambda"]).to(DEVICE)

        loss = torch.nn.CrossEntropyLoss()
        optimiser= torch.optim.RMSprop(model.parameters(), lr=config['learning_rate'])
        train_func= Model_Func.DF_train_rnn
        ...
        pickle.dump( model.return_DF_weights(), open(f"{save_path}/weights/{fold_name}-{feat}-fold{USERS[i]}-w-e{EPOCH}.pkl", "wb") )
        ...
        w_list.append(model.return_DF_weights())
    pickle.dump(fold_average, open(f"{save_path}/{fold_name}-multivar-avg_dict-e{EPOCH}.pkl", 'wb'))
    pickle.dump(np.array(w_list).mean(axis=0), open(f"{save_path}/{fold_name}-multivar-avg_w-e{EPOCH}.pkl", "wb"))
    return fold_average

if __name__=="__main__":

    parser= argparse.ArgumentParser(description='This script is used to produce the baselines for the Deep Feature Selection method with the Aggregated Hadmard product adapataion, and stores the training-validation split of the data for later use.')
    ...
    save_path= f"{default_path}{args.device_sensor}/{feature_setting}/DF"

...
def main():
    ...
    for i in range(len(USERS)):
        ...
        model= Models.Multivariate_IELayer(device=DEVICE, input_dim=[win_len, input_dim],
        ...
        pickle.dump( model.return_IE_weights(), open(f"{save_path}/weights/{fold_name}-{feat}-fold{USERS[i]}-w-e{EPOCH}.pkl", "wb") )
        ...
        w_list.append(model.return_IE_weights())
...
if __name__=="__main__":
    ...
    parser= argparse.ArgumentParser(description='This script is used to produce the baselines for PIEE\'s Weight-Naive Aggregated Hadamard product method, and stores the training-validation split of the data for later use.')
    ...
    save_path= f"{default_path}{args.device_sensor}/{feature_setting}/Weight"

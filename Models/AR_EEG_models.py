...
class LSTMEncoder_EEGNet_Wrapper(EEGNet_Wrapper):
    def __init__(self, device: torch.device, eegnet: torcheeg.models.EEGNet, input_dim:int):
        ...
        self.lstm= torch.nn.LSTM(input_dim[1],input_dim[1])
...
#         lstm_output=torch.permute(lstm_output, (1,2,0)).unsqueeze(1)
        lstm_output= self.dropout(lstm_output).unsqueeze(1)
#         print("lstm output swapped", lstm_output.shape)
        
        return self.eegnet(lstm_output)
    
class LSTM_EEGNet_Wrapper(EEGNet_Wrapper):
    def __init__(self, device: torch.device, eegnet: torcheeg.models.EEGNet, input_dim:int):
        ...
        for i in input_dim:
            assert isinstance(i, int), "Expected i to be an int"
        self.lstm= torch.nn.LSTM(input_dim[0],input_dim[0], batch_first=True)
        self.sigmoid= torch.nn.Sigmoid()
        self.dropout= torch.nn.Dropout(0.2)

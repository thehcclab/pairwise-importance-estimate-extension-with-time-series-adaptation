...
def main():
    ...
    baseline_model= UnivariateSimulationStudyBaseline_Model(device=device, input_dim=data.X.shape[1], classes=classes).to(device)
...
if __name__=="__main__":
    ...
    parser= argparse.ArgumentParser(description='This script is used to produce the baselines for PIEE\'s Gradient based methods method. It stores the training-validation split of the data for later use.')

    parser.add_argument('-dir', help='default path of where experiment is saved. E.g exp_log0', default="None")
    parser.add_argument('-opt', help="option for dataset to use or create. Options 0 or 1", default=0)
    args= parser.parse_args()
    args.dir= str(args.dir)
    args.opt= int(args.opt)

    feat= f"unisimul_study{args.opt}-Grad"
